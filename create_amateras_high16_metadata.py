...
def create_high16_metadata(folder_FITS, folder_metadata):
    """
    Parameters:
    folder_FITS: str
    ...
    for filename in list_file:  # iterate all files in folder_FITS
        ...
        if header["BITPIX"] != 16:
            print(folder_FITS + filename + " is not a 16 bit file")
            continue
        ...
        try:
            Time(header["DATE"])
            meta["granule_uid"] = (
                "iprt_amateras_high_16bit" + header["DATE"].replace("-", "") + "_v1.0"
            )  #
            meta["thumbnail_url"] = (
...
# Use in terminal python create_amateras_high16_metadata.py FOLDER_data folder_metadata
...
if __name__ == "__main__":
    ...
    try:  # Verify folder_metadata is given
        ...
        folder_metadata = "./high16metadata/"
    ...
    if os.path.isdir(folder_FITS):
        ...
        if all(os.path.isdir(os.path.join(folder_FITS, e)) for e in L_folder):
            ...
            for subfolder_FITS in L_folder:
                ...
                L_subfolder = os.listdir(folder_FITS + subfolder_FITS)
                if len(L_subfolder) == 0:
                    print(folder_FITS + subfolder_FITS + " is empty")
                    continue
                if all(
                    os.path.isdir(os.path.join(folder_FITS, subfolder_FITS, e))
                    for e in L_subfolder
                ):  # (doy folders)
                    print(
                        "Folders detected in {} {}/".format(folder_FITS, subfolder_FITS)
                    )

                    for subsubfolder_FITS in os.listdir(
                        os.path.join(folder_FITS, subfolder_FITS)
                    ):
                        if not os.path.isdir(
                            folder_metadata + subfolder_FITS + "/" + subsubfolder_FITS
                        ):
                            # if subfolder_FITS not in folder_metadata then create it.
                            os.makedirs(
                                os.path.join(
                        ...
                        create_high16_metadata(
                            os.path.join(folder_FITS, subfolder_FITS, subsubfolder_FITS)
                            + "/",
                            os.path.join(
                                folder_metadata, subfolder_FITS, subsubfolder_FITS
                            )
        ...
        create_high16_metadata(folder_FITS, folder_metadata)
